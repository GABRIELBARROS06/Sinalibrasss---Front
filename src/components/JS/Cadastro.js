...
function Cadastro() {
    ...
    const [textoErro,setTextoErro]=useState('')
        ...
        const resultadoData=dateString.slice(0,10)  
 const [dia, mes, ano] = resultadoData.split('/');
...
function validarEmail(email) {
  ...
  async function irParaQuizProfessor() {
    if (!emailProfessor || !validarEmail(emailProfessor)) {
        setErroNull(false);
